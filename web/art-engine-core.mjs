...
function techniqueMarks(ctx,event,points){
  ...
  const effect=String(event.renderEffect||'MATTE').toUpperCase(),depth=Math.max(0,Math.min(1,Number(event.paintDepth)||0));
  const baseAlpha=ctx.globalAlpha,baseWidth=event.width,baseStroke=ctx.strokeStyle;
  if(effect==='NEON'||tool==='neon_glow'){ctx.save();ctx.globalAlpha=Math.max(.10,baseAlpha*.22);ctx.lineWidth=baseWidth*(2.5+depth*2.6);ctx.shadowColor=baseStroke;ctx.shadowBlur=8+depth*24;strokePath(ctx,points);ctx.restore()}
  else if(effect==='IMPASTO'||tool==='impasto_heavy'||tool==='oil_lump'){ctx.save();ctx.globalAlpha=Math.max(.22,baseAlpha*.48);ctx.strokeStyle='rgba(0,0,0,.55)';ctx.lineWidth=baseWidth*(1.08+depth*.8);strokePath(ctx,points,1.5+depth*3,1.8+depth*3);ctx.restore();ctx.save();ctx.globalAlpha=Math.max(.16,baseAlpha*.38);ctx.strokeStyle='rgba(255,255,255,.78)';ctx.lineWidth=Math.max(.7,baseWidth*.18);strokePath(ctx,points,-.8-depth*1.4,-.8-depth*1.4);ctx.restore()}
  else if(effect==='CHROME'||tool==='chrome_ribbon'){ctx.save();ctx.globalAlpha=Math.max(.18,baseAlpha*.42);ctx.strokeStyle='rgba(0,0,0,.62)';ctx.lineWidth=baseWidth*1.45;strokePath(ctx,points,1.2,1.8);ctx.restore();ctx.save();ctx.globalAlpha=.72;ctx.strokeStyle='rgba(255,255,255,.92)';ctx.lineWidth=Math.max(.65,baseWidth*.18);strokePath(ctx,points,-.9,-.9);ctx.restore()}
  else if(effect==='AIRBRUSH'||tool==='airbrush_fog'){ctx.save();ctx.globalAlpha=Math.max(.07,baseAlpha*.18);ctx.lineWidth=baseWidth*3.8;ctx.shadowColor=baseStroke;ctx.shadowBlur=9;strokePath(ctx,points);ctx.restore()}
  else if(effect==='BLEED'||tool==='marker_bleed'){ctx.save();ctx.globalAlpha=Math.max(.12,baseAlpha*.26);ctx.lineWidth=baseWidth*2.1;strokePath(ctx,points,.8,.6);ctx.restore()}
...
function drawFullFly(ctx,tip,brushDown=false,time=0,inkColor='#4aa3a1',portraitOutline=false){
  ...
  const nearEdge=tx<82||tx>718||ty<82||ty>418;
  const nearCorner=(tx<112||tx>688)&&(ty<112||ty>388);
  // When the brush reaches an edge/corner the character visibly stops crawling,
  // lifts off, banks inward and flaps faster before settling at the next mark.
  const cornerLift=nearCorner?34+Math.sin(Number(time||0)/72)*8:(nearEdge?14:0);
  const inward=nearCorner?(tx>400?-1:1):0;
  const cx=clamp(tx-(nearCorner?44:70)+inward*18,82,718);
  const cy=clamp(ty-27-cornerLift,58,442);
  const dx=tx-cx,dy=ty-cy;
  const reach=Math.max(48,Math.hypot(dx,dy));
  const angle=Math.atan2(dy,dx);
  const bank=nearCorner?(tx>400?-.52:.52):(nearEdge?(tx>400?-.18:.18):0);
  const flap=nearCorner?Math.sin(Number(time||0)/32)*.46:(nearEdge?Math.sin(Number(time||0)/52)*.25:(brushDown?Math.sin(Number(time||0)/115)*.11:Math.sin(Number(time||0)/560)*.025));
  ...
  ctx.lineCap='round';
  ctx.lineJoin='round';
  const outline=false; // no white tracing: character reads through shape, not sticker outlines
  const outlineColor='transparent';

  // Transparent membrane wings: never white-filled. A faint cool tint and
  // restrained veins keep them readable over both the black UI and white canvas.
  const wing=(yScale,alpha,rotation)=>{
    ctx.save();
    ctx.translate(-5,-8);
    ctx.scale(1,yScale);
    ctx.rotate(rotation+flap*yScale);
    ctx.fillStyle=`rgba(116,156,166,${alpha})`;
    ctx.strokeStyle=`rgba(54,78,84,${Math.min(.30,alpha+.16)})`;
    ctx.lineWidth=1.15;
    ctx.beginPath();
    ctx.moveTo(1,0);
    ...
    // Keep the membrane transparent, but trace the outer silhouette in white
    // so the wings read as part of the same outlined JPGFLY character.
    if(outline){
    ...
    ctx.strokeStyle=`rgba(146,180,188,${Math.min(.24,alpha+.08)})`;
    ctx.lineWidth=.6;
    ctx.beginPath();
  ...
  if(outline)strokeLegs(outlineColor,3.25);
  strokeLegs('#111',2.1);
  ...
  ctx.closePath();
  ctx.fill();
  if(outline){ctx.strokeStyle=outlineColor;ctx.lineWidth=1.55;ctx.stroke()}
  ...
  // Bigger round head + oversized eye make the live character read as a
  // cartoon mascot at UI scale while the legs/brush still follow real marks.
  ctx.fillStyle='#0a0a0a';
  ctx.beginPath();
  ctx.ellipse(25,-2,20.5,20,-.05,0,TAU);
  ctx.fill();

  ctx.fillStyle='#f4f4ef';
  ctx.beginPath();
  ctx.ellipse(32,-5,10.3,12.8,-.14,0,TAU);
  ctx.fill();
  // No eye outline: at live-canvas scale its lower arc reads like a mouth.
  ctx.fillStyle='#090909';ctx.beginPath();ctx.ellipse(34,-4,5.2,6.7,-.12,0,TAU);ctx.fill();
  ctx.fillStyle='rgba(255,255,255,.92)';ctx.beginPath();ctx.arc(36.2,-7.1,1.7,0,TAU);ctx.fill();

  // JPGFLY artist beret. Keep it compact so the fly still reads cleanly
  // while moving quickly across the live canvas.
  ctx.save();
  ctx.translate(18,-17);
  ctx.rotate(-.12);
  // Fully opaque beret. Use a solid fill rather than a light gradient so
  // it never reads as translucent against the head or a white canvas.
  ctx.globalAlpha=1;
  ...
  ctx.lineWidth=1;
  ctx.beginPath();
  ctx.ellipse(0,0,15.5,5.8,0,0,TAU);
  ctx.fill();
  if(outline){ctx.strokeStyle=outlineColor;ctx.lineWidth=1.45;ctx.stroke()}
  ctx.strokeStyle='#3d2618';ctx.lineWidth=1;ctx.stroke();

  // Folded lower edge gives the hat a clear beret silhouette at small sizes.
  ctx.fillStyle='#5c3823';
  ctx.beginPath();
  ctx.ellipse(2,3,11.5,3.2,0,0,TAU);
  ctx.fill();

  // Tiny painter stem.
  ctx.strokeStyle='#4b2d1d';
  ctx.lineWidth=2.1;
  ctx.beginPath();
  ctx.moveTo(-2,-4.5);
  ctx.quadraticCurveTo(-1,-9,2,-10);
  ctx.stroke();

  // Painterly flecks: dusty turquoise, faded coral, mustard and cream.
  // They read as art-studio marks rather than neon UI decoration.
  ctx.fillStyle='#4aa3a1';
  ctx.beginPath();ctx.arc(-6,-1,1.15,0,TAU);ctx.fill();
  ctx.fillStyle='#d8785d';
  ctx.beginPath();ctx.arc(1.5,-2.1,1.05,0,TAU);ctx.fill();
  ctx.fillStyle='#d2a23b';
  ctx.beginPath();ctx.arc(7,.1,.95,0,TAU);ctx.fill();
  ctx.fillStyle='#e8d7b8';
  ctx.beginPath();ctx.arc(-1,1.5,.8,0,TAU);ctx.fill();
  ctx.restore();

  // Clean head: no mouth, antennae, bristles or fin-like facial strokes.

  // Front leg stays completely below/outside the head. Keep every stroked
  // segment out of the face ellipse so no moving arm segment can ever read
  // as a mouth on the live canvas.
  const gripX=Math.min(54,Math.max(48,reach*.60));
  const shoulder=[0,24],elbow=[22,31],grip=[gripX,26];
  ...
  if(outline){ctx.strokeStyle=outlineColor;ctx.lineWidth=3.55;frontLegPath()}
  ctx.strokeStyle='#111';ctx.lineWidth=2.4;frontLegPath();

  // Clean brush grip: foreleg and brush remain; split prongs are removed.

  // Traditional dark brush with a silver ferrule. The bristles stay
  // natural/dark; only a small amount of the current ink sits at the tip.
  ...
  if(outline){ctx.strokeStyle=outlineColor;ctx.lineWidth=6;handlePath()}
  ctx.strokeStyle='#171717';ctx.lineWidth=4.6;handlePath();
  ...
  ctx.lineWidth=.65;
  ctx.beginPath();
  ctx.moveTo(bristleStart,-1.5);ctx.lineTo(reach,0);
  ctx.moveTo(bristleStart,.2);ctx.lineTo(reach,0);
  ctx.moveTo(bristleStart,1.8);ctx.lineTo(reach,0);
  ctx.stroke();

  // Just a dab of ink at the very end — never a rainbow-loaded brush.
  const ink=String(inkColor||'#4aa3a1');
  ctx.fillStyle=ink;
  ctx.globalAlpha=brushDown?1:.82;
  ctx.beginPath();
  ctx.arc(reach-.6,0,brushDown?2.15:1.55,0,TAU);
  ctx.fill();
  ctx.globalAlpha=1;

  // FINAL FACE SEAL.
  // Repaint the complete head after every arm/brush stroke so absolutely no
  // earlier line can survive across the face and read as a mouth, smile, fin,
  // antenna or bristle. Then restore only the eye and opaque beret.
  ctx.save();
  ctx.globalAlpha=1;
  ctx.globalCompositeOperation='source-over';
  ...
  ctx.beginPath();
  ctx.ellipse(34,-4,5.2,6.7,-.12,0,TAU);
  ctx.fill();
  ctx.fillStyle='#fff';
  ctx.beginPath();
  ctx.arc(36.2,-7.1,1.7,0,TAU);
  ctx.fill();
...
export function renderFlyOverlay(ctx,tip,brushDown=false,time=0,inkColor='#4aa3a1'){ctx.save();ctx.setTransform(ctx.canvas.width/WIDTH,0,0,ctx.canvas.height/HEIGHT,0,0);drawFullFly(ctx,tip,brushDown,time,inkColor,false);ctx.restore()}
export function renderFlyPortrait(ctx,{time=0,inkColor='#4aa3a1',brushDown=false,background=null}={}){
  const logicalWidth=270,logicalHeight=180,tip=[195,102];
  ctx.save();
  ctx.setTransform(1,0,0,1,0,0);
  ctx.clearRect(0,0,ctx.canvas.width,ctx.canvas.height);
  if(background){ctx.fillStyle=background;ctx.fillRect(0,0,ctx.canvas.width,ctx.canvas.height)}
  const scale=Math.min(ctx.canvas.width/logicalWidth,ctx.canvas.height/logicalHeight);
  const offsetX=(ctx.canvas.width-logicalWidth*scale)/2;
  const offsetY=(ctx.canvas.height-logicalHeight*scale)/2;
  ctx.setTransform(scale,0,0,scale,offsetX,offsetY);
  drawFullFly(ctx,tip,brushDown,time,inkColor,false);
  ctx.restore();
}
